...
def test_run_processes_resources_with_schedule_tag(mock_discover_resources, mock_get_handler):
    """
    Test that the run method processes resources that have a schedule tag.
    """
    # Arrange
...
@patch('src.lambda_function.core.orchestrator.get_handler')
@patch.object(Orchestrator, 'discover_resources')
def test_run_calls_handler_for_resource(mock_discover_resources, mock_get_handler):
    """
