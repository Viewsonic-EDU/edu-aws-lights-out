...
describe('TagDiscovery', () => {
  ...
  describe('discover()', () => {
    ...
    it('should discover single resource with default priority and group', async () => {
      ...
      const discovery = new TagDiscovery({ 'lights-out:managed': 'true' }, ['ecs:service']);
      ...
      expect(res.arn).toBe('arn:aws:ecs:us-east-1:123456789012:service/my-cluster/my-service');
    ...
    it('should handle paginated API responses correctly', async () => {
      taggingMock
        ...
        .resolvesOnce({
          ResourceTagMappingList: [
            {
              ResourceARN: 'arn:aws:ecs:us-east-1:123456789012:service/cluster1/service1',
              Tags: [],
            },
          ],
        ...
        .resolvesOnce({
          ResourceTagMappingList: [
            {
              ResourceARN: 'arn:aws:ecs:us-east-1:123456789012:service/cluster2/service2',
              Tags: [],
            },
          ],
          PaginationToken: undefined,
      ...
      expect(arns).toContain('arn:aws:ecs:us-east-1:123456789012:service/cluster1/service1');
      expect(arns).toContain('arn:aws:ecs:us-east-1:123456789012:service/cluster2/service2');
    });
  });
  ...
  describe('ARN parsing', () => {
    it.each([
      ...
      ['arn:aws:rds:eu-west-1:123456789012:db:my-db-instance', 'rds:db', 'my-db-instance', {}],
      [
    ...
    it('should discover resources across multiple regions', async () => {
      ...
      taggingMock.on(GetResourcesCommand).resolves({
        ResourceTagMappingList: [
          {
            ResourceARN: 'arn:aws:ecs:ap-southeast-1:123456789012:service/cluster-sg/service-sg',
    ...
    it('should handle errors in one region without affecting others', async () => {
      ...
      taggingMock.on(GetResourcesCommand).callsFake(() => {
        ...
        if (callCount === 1) {
          ...
          return {
            ResourceTagMappingList: [
              {
                ResourceARN: 'arn:aws:ecs:ap-southeast-1:123456789012:service/cluster/service',
                Tags: [],
              },
            ],
            PaginationToken: undefined,
      ...
      const discovery = new TagDiscovery({}, ['ecs:service'], ['ap-southeast-1', 'ap-northeast-1']);
